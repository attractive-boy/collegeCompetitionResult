...
export default async function getNotice(
  ...
  if (searchValue !== undefined) {
    const sql = `SELECT id,year,project,level,work,student,college,teacher,team,photo FROM achievement WHERE project LIKE ? or work LIKE ? or student LIKE ? or teacher LIKE ? ORDER BY id DESC`;
    // 学生和老师精确查询 用逗号隔开的
    const values = [
        `%${searchValue}%`,
        `%${searchValue}%`,
  ...
  const query = `SELECT id,year,project,level,work,student,college,teacher,team,photo FROM achievement ORDER BY id DESC`;
